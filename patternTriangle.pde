class PatternTriangle {
  ...
  PatternTriangle(PVector i_pt1, PVector i_pt2, PVector i_pt3, PImage texture) {

    pt1 = i_pt1;
    pt2 = i_pt2;
    pt3 = i_pt3;

    float maxX = max(i_pt1.x, i_pt2.x, i_pt3.x);
    float minX = min(i_pt1.x, i_pt2.x, i_pt3.x);
    float maxY = max(i_pt1.y, i_pt2.y, i_pt3.y);
    float minY = min(i_pt1.y, i_pt2.y, i_pt3.y);
